...
st.dataframe(df_f[cols_show], use_container_width=True, height=420)        return SPECIAL_ISO3[tok]
...
with left:
    st.subheader("CDM Activity by Host Party")
    topn = st.slider("Top N (bar)", min_value=5, max_value=30, value=10, step=1)
...
# ======================
# PIE: Emission reductions by Host Country (filtered)
# ======================
st.subheader("Emission Reductions by Host Country (ktCO2e/yr)")

if "Reductions (ktCO2e/yr)" in df_f.columns and "Host country" in df_f.columns:
    pie_df = df_f.copy()  # ✅ ikut filter
    pie_df["reductions"] = pd.to_numeric(pie_df["Reductions (ktCO2e/yr)"], errors="coerce").fillna(0)
...
st.dataframe(df_f[cols_show], use_container_width=True, height=420)    if len(tok) == 2 and tok.isalpha():
        c = pycountry.countries.get(alpha_2=tok.upper())
        return c.alpha_3 if c else None
