...
# =========================
BLUE_SCALE = [
    [0.0, "#cfe2ff"],  # light blue (low/zero)
    [1.0, "#08306b"],  # dark blue (high)
]

# For binary (0/1) layers we still want the same logic:
BLUE_BINARY = [
    [0.0, "#cfe2ff"],  # 0 -> light blue
    [1.0, "#08306b"],  # 1 -> dark blue
]
...
def apply_map_style(fig):
    """Consistent 'clean policy-report' styling + light->dark blue scale already applied in px."""
    fig.update_geos(
        showcoastlines=True,
...
# ---- Choose metric for coloring (ALL use light->dark blue)
if map_choice == "Total mechanisms (0–8)":
    ...
    fig_map = apply_map_style(fig_map)
    ...
    fig_map = px.choropleth(
        ...
        color="present",
        hover_name="Country",
        color_continuous_scale=BLUE_BINARY,
    )
...
with tab1:
    ...
    with c2:
        ...
        if len(v) == 0:
            ...
            st.plotly_chart(
                px.bar(top, x="Country", y="vcm_projects"),
                use_container_width=True,
                key="bar_vcm",
            )
