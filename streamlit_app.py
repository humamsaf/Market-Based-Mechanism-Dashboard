...
def load_cbam_data():
    ...
    df["_product_label"] = df["ProductCode"] + " — " + df["Product Description"]
    return df
...
def page_cbam():
    ...
    total_val_m = df_real["Trade Value 1000USD"].sum() / 1_000       # million USD
    eu_val_m    = df_real[df_real["Reporter"] == "European Union"]["Trade Value 1000USD"].sum() / 1_000
    uk_val_m    = df_real[df_real["Reporter"] == "United Kingdom"]["Trade Value 1000USD"].sum() / 1_000
    total_val   = total_val_m / 1_000   # billion USD (hero summary)
    eu_val      = eu_val_m    / 1_000
    ...
    fc1, fc2, fc3, fc4, fc5 = st.columns([1.2, 1.2, 1.8, 2, 0.6])
    with fc1:
    ...
    with fc4:
        ...
        # Extract just the codes from selected labels
        product_codes_sel = [lbl.split(" — ")[0] for lbl in product_sel]
    with fc5:
        st.markdown('<div style="height:28px"></div>', unsafe_allow_html=True)
    ...
    if product_codes_sel:  f = f[f["ProductCode"].isin(product_codes_sel)]
    ...
    # Palette for product-mode (up to 20 products)
    PRODUCT_PALETTE = [
        "#1d3557","#e63946","#2a9d8f","#e07b00","#9b59b6",
        "#4a90d9","#f4a261","#264653","#e9c46a","#a8dadc",
        "#457b9d","#d62828","#023e8a","#80b918","#f72585",
        "#7209b7","#3a0ca3","#4cc9f0","#b5838d","#6b705c",
    ]
...
def page_ets():
    ...
    with col_card:
        ...
        if not selected:
            ...
            # Compute share + price for header badges (use first/selected scheme)
            _r0 = schemes_display.iloc[0] if not schemes_display.empty else None
            _share_hdr = ""
            _price_hdr = ""
            if _r0 is not None:
                try:
                    _s = float(_r0.get("share"))
                    _share_hdr = f"{_s*100:.0f}%"
                except: pass
                try:
                    _p = fval(_r0.get("price"))
