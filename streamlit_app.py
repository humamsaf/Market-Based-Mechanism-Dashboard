...
def page_ets():
    ...
    def stat(number, label, sub=None):
        ...
        return (
            f'<div style="text-align:center;">'            f'<div style="font-size:56px;font-weight:900;color:#1a1a2e;line-height:1;white-space:nowrap;">{number}</div>'            f'<div style="font-size:11px;color:#999;font-weight:700;text-transform:uppercase;letter-spacing:2px;margin-top:8px;white-space:nowrap;">{label}</div>'            f'{sub_html}</div>'
        )
    ...
    # ── Timeline-file app with top navbar via query_params routing
from __future__ import annotations
import streamlit as st
import pandas as pd
import plotly.express as px
